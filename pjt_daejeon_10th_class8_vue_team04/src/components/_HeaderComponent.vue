  ...
  <div class="header-wrapper flex bg-white justify-between px-4">
    ...
    <div class="flex items-center" v-if="!user">
      <router-link to="/login">
        <BaseBtn class="bg-purple-500 mb-2 me-5 text-white rounded-full">Login</BaseBtn>
      </router-link>

      <router-link to="/register">
        <BaseBtn class="bg-purple-500 mb-2 me-3 text-white rounded-full">Join</BaseBtn>
      </router-link>
    ...
    <div class="flex items-center" v-else>
      <BaseBtn class="bg-purple-500 mb-2 me-5 text-white rounded-full" @click="onLogoutClick"
        >Logout</BaseBtn
      >

      <router-link to="/edit">
        <BaseBtn class="bg-purple-500 mb-2 me-3 text-white rounded-full">Mypage</BaseBtn>
        <!-- <BaseBtn as="div" class="bg-purple-500 mb-2 me-3 text-white rounded-full"></BaseBtn> -->
      </router-link>
    </div>
  </div>
</template>
...
<!-- <template>
  <v-app-bar>
    ...
    <v-app-bar-title>
      <router-link to="/"> <v-btn :variant="`plain`">Title</v-btn> </router-link>
    </v-app-bar-title>
    <v-spacer></v-spacer>
    <v-item-group v-if="!user">
      <router-link to="/login"><v-btn :variant="`plain`">로그인</v-btn></router-link>
      <router-link to="/register"><v-btn :variant="`plain`">회원가입</v-btn></router-link>
    </v-item-group>
    <v-item-group v-else>
      <v-btn :variant="`plain`" @click="onLogoutClick">로그아웃</v-btn>
      <router-link to="/edit"><v-btn :variant="`plain`">마이페이지</v-btn></router-link>
    </v-item-group>
